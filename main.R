...
db.host <- "airbnbrdb.cvuvrunlpgtc.us-west-2.rds.amazonaws.com"
db.password <- "waterice1"
db.user <- "sean"
db.dbname <- "airbnbdb"
db.port <- 5432
...
getCityInfo <- function(city) {
    # Get basic metrics of Airbnb market in each city. This function
    # will help investor to make decision when deciding which city
    # to invest.
    
    dat <- GetDataWithScrapeId("detail_page", 16)

    showMedianRateAndOccu <- function() {
        dat.aggr <- dat %>%
            group_by(listing_id) %>%
            summarise(price = mean(price),
                      occupancy = 1 - mean(availability))

        price.med <- median(dat.aggr$price)
        occupancy.med <- median(dat.aggr$occupancy)

        cat(sprintf("
City %s
Median daily rate is %d.
Median occupancy rate is %d%%

",
           city,
           round(price.med),
           round(occupancy.med * 100)))
    }
}



g1 <- ggplot(aes(x = price), data = dat.list) +
    geom_histogram() +
